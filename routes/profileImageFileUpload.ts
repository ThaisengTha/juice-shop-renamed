    ...
    if (uploadedFileType === undefined) {
      ...
      if (uploadedFileType !== null && utils.startsWith(uploadedFileType.mime, 'image')) {
        ...
        if (loggedInUser) {
          ...
          const safeExt = path.basename(String(uploadedFileType.ext)).replace(/(\.\.(\/|\\))/g, '')
          fs.open(`frontend/dist/frontend/assets/public/images/uploads/${safeId}.${safeExt}`, 'w', function (err, fd) {
